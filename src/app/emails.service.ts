...
export class EmailsService {
  ...
  private snoozeEmailSnackBar(type: 'tomorrow' | 'this_weekend' | 'next_week' | 'some_day') {
    ...
    switch (type) {
      case 'tomorrow':
        snackBarRef = this.snackBar.open('Snoozed until tomorrow', 'Undo', { duration: 7000 });
        break;
      case 'this_weekend':
        snackBarRef = this.snackBar.open('Snoozed until this weekend', 'Undo', { duration: 7000 });
        break;
      case 'next_week':
        snackBarRef = this.snackBar.open('Snoozed until next week', 'Undo', { duration: 7000 });
        break;
      case 'some_day':
        snackBarRef = this.snackBar.open('Snoozed until some day', 'Undo', { duration: 7000 });
        break;
      default:
        throw new ReferenceError('Invalid param');
    }
    snackBarRef.onAction().subscribe(() => {
      this.snackBar.open('Snooze was reverted', null, { duration: 6000 });
    });
  }
  /**
  ...
  generateDummyContent() {
    ...
    const htmlEmailContent = `<h1>This is header 1</h1>
